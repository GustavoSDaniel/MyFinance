...
public interface TransactionRepository extends JpaRepository<Transaction, UUID>, JpaSpecificationExecutor<Transaction> {
     ...
     * <p>
     * O intervalo é inclusivo em ambas as extremidades ({@code createdAt BETWEEN startDate AND endDate}).
     * O resultado é uma lista de objetos {@link DashboardCategorySum}, onde cada elemento contém
     ...
     * @param startDate Data e hora inicial do período (inclusiva)
     * @param endDate   Data e hora final do período (inclusiva)
     * @return Lista de {@link DashboardCategorySum} com o total de despesas por categoria no período.
     *         Pode ser vazia se não houver despesas.
     */
    @Query("""
           SELECT new com.gustavosdaniel.myfinance_api.domain.dto.request.DashboardCategorySum(
                      c.name, 
                      c.color, 
                      SUM(t.amount)
           )
